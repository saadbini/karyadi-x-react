  ...
  const fetchApplicants = async () => {
    try {
      ...
      const filteredApplicants =
        user?.userType === "admin"
          ? allApplicants
          : allApplicants.filter((applicant) => applicant.job_posts.created_by === user?.id);
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-100">
      <header className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Candidate Listing</h1>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {applicants.map((applicant) => (
                <tr key={applicant.id} className="hover:bg-gray-100">
                  <td className="p-3 text-gray-900">{applicant.full_name}</td>
                  <td className="p-3 text-gray-600">{applicant.job_post.title}</td>
                  <td className="p-3 text-gray-600">{applicant.email}</td>
